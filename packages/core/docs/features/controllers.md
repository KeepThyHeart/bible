...
- **`SearchController` has no search-history methods.** The app does not record what users search for, so the surface is absent rather than present-and-no-op - see [Search](search.md).
