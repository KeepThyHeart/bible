/**
 * Pure verse-text formatting: raw module text plus word-range metadata in,
 * display-ready HTML out.
 *
 * Deliberately free of the `BibleVerse` model (and so of the Data layer): the
 * browser barrel exports this file, which lets the web client's offline
 * (OPFS/wa-sqlite) read path render verses with the very same code the server
 * uses, instead of a hand-maintained copy that can drift. `VerseFormatter.ts`
 * is the thin `BibleVerse` adapter over it.
 */
...
/** A word range, 0-based and inclusive, into the tag-free word sequence. */
...
/**
 * The `formatting_data` payload as stored in module databases. Both spellings
 * are accepted because rows in the wild carry either (the desktop/server side
 * projects camelCase, the web offline worker reads raw snake_case columns).
 */
export interface VerseFormattingData {
  paragraphStart?: boolean;
  paragraph_start?: boolean;
  wordsOfChrist?: VerseWordRange[];
  words_of_christ?: VerseWordRange[];
  /** Tetragrammaton word ranges (module format v2 `divine_name` span). */
  divineName?: VerseWordRange[];
  divine_name?: VerseWordRange[];
  sectionHeading?: string;
...
/**
 * Formats raw verse text into display-ready HTML.
 *
 ...
 *
 * @param sectionHeading Overrides the heading read from `fd`. `formatVerseText`
 *   passes `BibleVerse.getSectionHeading()` here because a verse's structured
 *   `formatting.block.heading` takes precedence over the legacy payload.
 */
export function formatVerseFields(
  text: string,
  fd: VerseFormattingData | undefined,
  sectionHeading?: string,
): FormattedVerse {
  ...
  const rawHeading = sectionHeading ?? fd?.sectionHeading ?? fd?.section_heading;
  const heading = rawHeading ? rawHeading.replace(/<[^>]*>/g, '').trim() || undefined : undefined;

  return { textHtml: text, isParagraphStart, sectionHeading: heading };
}
...
/** True when the formatting data carries any Words of Christ ranges. */
export function hasWordsOfChrist(fd: VerseFormattingData | undefined): boolean {
  const woc = fd?.wordsOfChrist || fd?.words_of_christ;
  return (woc?.length ?? 0) > 0;
}

/** The footnotes in the formatting data, or an empty list. */
export function getFootnotes(
  fd: VerseFormattingData | undefined,
): Array<{ position: number; marker: string; text: string }> {
  return fd?.footnotes ?? [];
}
