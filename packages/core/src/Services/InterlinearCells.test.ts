 ...
 * the shipped `bible_kjv.db` module, so a change in the shipped
...
import { extractWordsWithFormatting, type WordInfo } from './WordIndexing';
...
// ---------------------------------------------------------------------------
// Cases ported from the web app's former copy of this suite. They use a short
// synthetic verse and also cover `cellStrongsNumbers`.
// ---------------------------------------------------------------------------

const SMALL_VERSE = 'For God so loved the world';

function smallWords(html = SMALL_VERSE) {
  return extractWordsWithFormatting(html);
}

function smallRow(overrides: Partial<InterlinearWord> = {}): InterlinearWord {
  return {
    wordPositionStart: 0,
    wordPositionEnd: 0,
    originalWord: 'θεός',
...
describe('buildInterlinearCells - small synthetic verse (ported from the web suite)', () => {
  it('covers every English word exactly once', () => {
    const english = smallWords();
    const cells = buildInterlinearCells(english, [
      smallRow({ wordPositionStart: 1, wordPositionEnd: 1 }),
    ]);
  ...
  it('orders cells by the English text, not by row order', () => {
    const english = smallWords();
    const cells = buildInterlinearCells(english, [
      smallRow({ wordPositionStart: 3, wordPositionEnd: 3, gloss: 'loved', strongsNumber: 'G25' }),
      smallRow({ wordPositionStart: 1, wordPositionEnd: 1 }),
    ]);
  ...
  it('groups a multi-word gloss into one cell', () => {
    const english = smallWords();
    const cells = buildInterlinearCells(english, [
      smallRow({ wordPositionStart: 2, wordPositionEnd: 3, gloss: 'so loved' }),
    ]);
  ...
  it('pins gloss-less rows to the cell owning their position as extra Strongs', () => {
    ...
    const english = smallWords();
    const cells = buildInterlinearCells(english, [
      smallRow({ wordPositionStart: 0, wordPositionEnd: 0, gloss: 'For', strongsNumber: 'G1063' }),
      smallRow({ wordPositionStart: 0, wordPositionEnd: 0, gloss: '', strongsNumber: 'G3588' }),
    ]);
  ...
  it('demotes out-of-range rows instead of dropping their Strongs number', () => {
    const english = smallWords();
    const cells = buildInterlinearCells(english, [
      smallRow({ wordPositionStart: 99, wordPositionEnd: 99, strongsNumber: 'G9999' }),
    ]);
    expect(cellsPartitionWordSpace(cells, english.length)).toBe(true);
  ...
  it('falls back to claiming by position when no row has a gloss', () => {
    const english = smallWords();
    const cells = buildInterlinearCells(english, [
      smallRow({ wordPositionStart: 0, wordPositionEnd: 1, gloss: '', strongsNumber: 'G1' }),
      smallRow({ wordPositionStart: 2, wordPositionEnd: 5, gloss: '', strongsNumber: 'G2' }),
    ]);
  ...
  it('carries christ-words and divine-name formatting onto the tokens', () => {
    const english = smallWords('<span class="christ-words">For God</span> so <span class="divine-name">Lord</span>');
    const cells = buildInterlinearCells(english, [smallRow({ wordPositionStart: 1, wordPositionEnd: 1 })]);
