/**
 * DTO shapes for the data-provider interfaces (`./interfaces.ts`) - the
 * JSON-over-the-wire (or over-IPC) shape of Bible/commentary/search/... data,
 * as distinct from the SQL-shaped module repositories in `Data/Repositories/`.
 *
 * Promoted from `apps/web/src/types.ts` into `packages/core` (task 0034,
 * finishing 0029's S3a: "promote the data-provider interfaces into
 * packages/core as the app-wide content-source seam"). Pure data, no
 * platform dependency - safe in `@bible/core/browser` alongside the
 * interfaces that use it, so a desktop adapter (`apps/desktop/electron/src`)
 * can construct the same DTOs from its own IPC surface without depending on
 * the web app.
 */
