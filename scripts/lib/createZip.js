#!/usr/bin/env node
/**
 * A minimal, dependency-free ZIP writer for `scripts/build-module-pack.js`.
 *
 * This is a deliberate, close port of `packages/extension-testing/src/cli/
 ...
 * The two implementations are small (~80 lines) and encode a fixed, simple
 * subset of the ZIP format, so duplicating them is a smaller risk than a
 * cross-package build-order dependency; if the format needs to change, both
 * copies need the same change; this comment is the tripwire for that.
 *
 * Node ships `zlib`, and the format needed here is one method (deflate) and
 * three record types (local header, central directory, end of central
 * directory), so it is written out rather than pulling in `archiver`/`jszip`
 * as a dependency.
 *
 ...
 * produces the same bytes and therefore the same SHA-256.
 *
...
/** 1980-01-01 00:00:00, the earliest the DOS date format can express. Fixed for reproducibility. */
const DOS_TIME = 0;
 ...
 * @param {Array<{path: string, content: Buffer}>} entries - `path` is
 *   forward-slash separated, never a leading `/`.
 * @returns {Buffer}
 */
