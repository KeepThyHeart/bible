...
* **Book names and reference parsing.** The largest functional gap: modules carry no book names, so "Juan 3:16" does not parse. **The interface is implemented** (`packages/core/src/Data/Locales/Localizer.ts`'s `referenceParserConfig` field, registered per tag via `registerLocalizer()`/`getLocalizer()`), and **every desktop call site that used to build a `ReferenceParser` (or read `LONG_NAMES`/`ENGLISH_BOOK_NAMES` for display) with no locale awareness now goes through it** - see `apps/desktop/src/ui/services/localizedReferenceParser.ts` and `apps/desktop/electron/services/localizedReferenceParser.ts` for the two (renderer / main-process) accessors, and `constants/bibleBooks.ts`'s `localizedBookNames()`/`localizedBookAliases()`/`localizedAllBooks()` for display. `packages/core/src/Services/CommentaryLinkProcessor.ts` is the one deliberate exception: it parses references inside already-shipped (English-only) commentary/dictionary module text, a *module content* language, not the UI language, so it stays on the English table on purpose - the same "content direction is independent of UI direction" principle already applied to Bible text elsewhere. **Still missing: no language's book-name table has been drafted** - that is real content work (66 book names × long/short/aliases, per language) left for a dedicated future pass; every locale falls back to the English table today via `EnglishLocalizer`, which is exactly the "English always accepted as a parsing fallback" behavior the roadmap calls for - it is just currently the *only* table that exists. Web's own parsing (`apps/web/src/components/BiblePane/BookChapterPicker.tsx`, `Dialogs/CopyDialog.tsx`) uses hand-rolled parsers with an English-only alias table (`constants.ts`'s `BOOK_ALIASES`) rather than core's `ReferenceParser`, by design (see the comments in those files) - that gap was not touched in this pass and is left for whoever picks up web-side reference parsing.
