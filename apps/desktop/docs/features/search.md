...
| `packages/core/src/Services/SemanticSearchService.ts` | Similarity search over the embedding index, with `minSimilarity` and level filtering |
| `packages/core/src/Services/WordFamilyService.ts` | Strong's word-family expansion behind `includeRelatedWords` |
| `packages/core/src/Data/Core/FeaturePackTypes.ts` | Catalog + package schemas and fail-closed validators (artifact path safety, digests, size ceilings) |
...
- **No in-repo packaging script for a semantic feature pack.** The app installs and validates packs (catalog, file and folder), but the tooling that stages, hashes and emits `feature-pack.json` for a new pack is not part of this repository. `packages/core/src/Data/Core/FeaturePackTypes.ts` is the authoritative description of the shape such a pack must have.
