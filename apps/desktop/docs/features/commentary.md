...
`constants.ts`'s `DEFAULT_COMMENTARY_ABBREVIATION`, the `electron-builder*.yml` file lists, and `ModuleCard.tsx` (Module Manager, where the real abbreviation is the point) keep the raw string deliberately.
