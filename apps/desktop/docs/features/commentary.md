...
| `src/ui/moduleDescriptions.ts` | The catalog keys for the provenance notice, plus `DIGEST_DISPLAY_NAME` ("Combined Summary"); re-exports the detection rules (`isDigestModule()`, `getModuleProvenanceKind()`, ...) from `packages/core/src/Services/ModuleDescriptions.ts`, which the web app shares |
