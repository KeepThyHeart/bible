...
**Bundled packs.** The recommended sets live in one file, `packages/core/src/Data/Core/starter-packs.json` (exported as `BUNDLED_STARTER_PACKS`). Each pack is only a list of catalog `module_ids` plus a name, description and the languages it is offered for - nothing is bundled but the list. The official, verified catalog offers every bundled pack it does not already publish itself under the same `pack_id` (a catalog's own `starter_packs` win, so its maintainers can change a list without an app release), and each id is resolved against that catalog like any other pack. To add a tier, append an object to `packs`; first run then lists it as another card, each with per-module checkboxes. `npm run init:modules -- --select=<pack_id>` installs the same list from the setup script. The shipped `essentials` pack is English-only; other languages fall back to the recommended modules below.
