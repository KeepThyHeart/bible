...
| `src/ui/components/BookChapterPicker.tsx` | Modal dialog frame (overlay, theme surface, `role="dialog"`) around the shared `BookChapterPicker` from `@bible/ui` (`packages/ui`); supplies i18n labels, localized names/aliases, the section tint (`sectionButtonStyle`) and the search-store results |
