...
/**
 * Everything a presenter needs but not mid-sentence: the running order,
 * hymns, screen settings and the join/handoff panel.
 *
 * Shared by two hosts, which is why it takes no opinion on how it is framed:
 *
 *  - `PresentPanel` wraps it as a floating popup behind the bottom strip's
 *    last button (phones, and any layout with no room for a permanent tab).
 *  - `PresentTab` embeds it directly in the Study pane's Present tab on
 *    desktop, where it is simply always there while that tab is selected.
 *
 * Splitting it out is the whole layout decision made once: the strip (or the
 * tab's own top row) holds what might be wanted mid-sentence, and this holds
