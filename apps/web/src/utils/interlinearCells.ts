 ...
 * sequence. Verified across all eight shipped modules that carry interlinear
 * data (~2.9M rows): for every row with a gloss,
 * `gloss === englishWords.slice(start, end + 1).join(' ')`.
 *
 * Because of that, the interlinear view does not need its own word model — it
 * can render *the English tokens themselves*, in the order the translation
 * puts them.
 *
 * A **cell** is one column of the interlinear stack (or one parenthetical of
 * the inline layout): a contiguous run of English tokens plus the interlinear
 * row, if any, that backs them.
 *
 * ## What this fixes
 *
 * The previous renderer emitted one block per interlinear row and nothing
 * else, so rows sharing a position rendered in arbitrary order and any English
 * word no row claimed was **silently dropped** — 5.7% of KJV, including every
 * italicised supplied word, and over half of RWebster. Emitting a
 * `source: null` cell for each unclaimed run is what puts those words back.
 *
 * NOTE: ported from the desktop app's `src/ui/components/study/interlinearCells.ts`
 * and kept in sync by hand. Sharing it via `@bible/core` would mean editing
 * that package's barrel exports, which is out of scope for this change.
 */

import type { WordInfo } from './wordIndexing';

/**
 * One row of a module's `interlinear_word` table, in the shape the cell
 * builder needs. `InterlinearWordData` from the API maps onto it directly.
 */
 ...
 * @param rows The verse's interlinear rows, in stored order
 */
 ...
 * back to the plain verse renderer instead of showing a verse with words
