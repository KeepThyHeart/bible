 ...
 *
 * This file lives under `src/present/`, alongside `protocol.ts`, rather than
 * under the server, so a client can apply the very same transition a server
 * would -- the one piece the same-machine transport (`transport/`) needs in
 * order to predict what the server is about to confirm, without a second copy
 * of this logic to keep in sync. It stays exactly as pure as that requires: no
 * database, no Express, no clock here either. `apps/web/server/present/
 * reducer.ts` re-exports this module so existing server imports are unaffected.
 */
...
export function validateIntent(value: unknown): PresentIntent | null {
  ...
  switch (value.type) {
    ...
    case 'setTheme':
      if (value.theme !== 'light' && value.theme !== 'dark' && value.theme !== 'max') return null;
