...
| `src/components/BiblePane/BookChapterPicker.tsx` | Modal dialog frame (overlay, size, phone full-screen) around the shared `BookChapterPicker` from `@bible/ui` (`packages/ui`), which does the book/chapter grids, reference textbox and Escape handling. This file supplies the i18n labels, localized book names, the search store results, the book topics under the chapter grid, and the target-translation line with its "Select Translation" link. Styled by `.kth-picker*` (packages/ui/css) plus `_book-chapter-picker.scss` for the frame, section colours (`data-section`), topics and translation line |
